...
class Eddsa {
    ...
    prvTopub(prv) {
        const F = this.babyJub.F;
        const sBuff = this.pruneBuffer(crypto.createHash('sha256').update(Buffer.from(prv)).digest());
        let s = Scalar.fromRprLE(sBuff, 0, 32);
        const A = this.babyJub.mulPointEscalar(this.babyJub.Base8, Scalar.shr(s, 3));
        return A;
    }
    ...
    signPoseidon(prv, msg) {
        ...
        const composeBuff = new Uint8Array(32 + msg.length);
        composeBuff.set(sBuff.slice(0, 32), 0);
        F.toRprLE(composeBuff, 32, msg);
        const rBuff = crypto.createHash('sha256').update(Buffer.from(composeBuff)).digest();
        let r = Scalar.mod(Scalar.fromRprLE(rBuff, 0, 32), this.babyJub.subOrder);
        const R8 = this.babyJub.mulPointEscalar(this.babyJub.Base8, r);
